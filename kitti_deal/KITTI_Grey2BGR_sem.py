...
WSI_MASK_PATH = "F:/Dataset/KITTI2015/training/instance/"

pre_path = 'F:/Dataset/KITTI2015/training/gtFine/'
...
def access_pixels(name, frame):
    ...
    for row in range(height):  # 遍历高
        for col in range(weight):  # 遍历宽
            for label in labels:
                if frame[row, col, 0] == label.id:  # 全部像素取反，实现一个反向效果

                    frame[row, col, 0] = label.color[2]
                    frame[row, col, 1] = label.color[1]  # 全部像素取反，实现一个反向效果
                    frame[row, col, 2] = label.color[0]  # 全部像素取反，实现一个反向效果
